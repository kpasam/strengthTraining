...
// Hardcoded labels for real exercises
const EXERCISE_LABELS: Record<string, Label> = {
...
// Patterns that indicate garbage (not a real exercise)
...
function seed() {
  ...
  const tx = sqlite.transaction(() => {
    ...
    for (const ex of exercises) {
      ...
      if (label) {
        upsert.run(ex.id, 1, label.bodyPart, label.intensity, label.movementType, label.equipment);
        labeled++;
      } else if (isGarbage(ex.canonical_name)) {
        upsert.run(ex.id, 0, null, null, null, null);
        garbage++;
      } else {
        // Unknown exercise — mark as real but with no labels so it still shows up
        upsert.run(ex.id, 1, null, null, null, null);
        unlabeled++;
      }
    }

    console.log(`Seeded exercise labels:`);
    console.log(`  Labeled: ${labeled}`);
    console.log(`  Garbage: ${garbage}`);
    console.log(`  Unlabeled (kept as exercise): ${unlabeled}`);
  });
